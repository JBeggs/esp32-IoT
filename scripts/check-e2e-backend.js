  ...
  try {
    ...
    if (!res.ok) {
      ...
      if (res.status === 401) {
        ...
        console.error('');
        console.error('Seed the esp32-iot tenant in django-crm first.')
        console.error('');
        process.exit(1);
      }
    ...
    const productsRes = await fetch(`${apiUrl}/v1/public/esp32-iot/products/?page_size=1`)
    if (!productsRes.ok) {
      console.error('');
    ...
    if (!Array.isArray(products) || products.length === 0) {
      console.error('');
      console.error('E2E backend not ready: no products for esp32-iot.')
      console.error('');
      console.error('Seed the esp32-iot tenant via the django-crm admin or fixtures.')
      console.error('');
      process.exit(1);
    }
