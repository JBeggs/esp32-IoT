...
export default async function ProductsPage({ searchParams }: ProductsPageProps) {
  ...
  return (
    <div className="min-h-screen tech-page-bg" data-cy="products-section">
      ...
      <section className="paper-section py-6 border-b border-[rgb(var(--color-paper-border))]">
        <div className="container-wide">
          <div className="flex flex-col gap-4">
            <div className="flex flex-col sm:flex-row gap-4 items-stretch sm:items-center">
              <form method="get" action="/products" className="relative max-w-md flex-1">
                ...
                <input
                  ...
                  className="w-full pl-10 pr-4 py-2.5 border border-[rgb(var(--color-paper-border))] rounded-lg bg-white text-slate-950 placeholder:text-slate-400 focus:ring-2 focus:ring-primary focus:border-primary"
                />
              </form>
            ...
            <div className="flex flex-col gap-3">
              {shelves.length > 0 && (
                <>
                  <div className="flex items-center gap-2 text-sm paper-muted">
                  ...
                  <div className="flex flex-wrap gap-2">
                    <Link
                      ...
                      className={`px-4 py-2 rounded-full text-sm font-medium transition-colors flex items-center gap-1 ${
                        isAllShelves ? 'bg-primary text-[rgb(var(--color-primary-fg))]' : 'bg-slate-100 text-slate-800 hover:bg-slate-200'
                      }`}
                    >
                      All
                    </Link>
                    {shelves.map((shelf) => {
                      ...
                      return (
                        <Link
                          ...
                          className={`px-4 py-2 rounded-full text-sm font-medium transition-colors flex items-center gap-1 ${
                            active ? 'bg-primary text-[rgb(var(--color-primary-fg))]' : 'bg-slate-100 text-slate-800 hover:bg-slate-200'
                          }`}
                        >
              ...
              {categories.length > 0 && (
                <div className="flex flex-col gap-2 pt-4 mt-2 border-t border-[rgb(var(--color-paper-border))]">
                  <div className="flex items-center gap-2 text-sm paper-muted">
                      ...
                      return (
                        <Link
                          ...
                          className={`px-4 py-2 rounded-full text-sm font-medium transition-colors ${
                            isCatActive ? 'bg-primary text-[rgb(var(--color-primary-fg))]' : 'bg-slate-100 text-slate-800 hover:bg-slate-200'
                          }`}
                        >
                          {cat.name}
                        </Link>
