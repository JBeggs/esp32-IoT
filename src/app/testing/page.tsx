    ...
    return (
      <div className="rounded-xl bg-amber-500/10 border border-amber-500/30 p-6 text-amber-800">
        ...
        <p className="text-sm opacity-90 mb-2">
          From the <code className="bg-black/10 px-2 py-1 rounded">esp32-iot</code> directory, run:
        </p>
    ...
    <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-4">
      {metrics.map(({ label, data: d, color }) => {
        ...
        return (
          <div
            key={label}
            className="paper-surface rounded-xl p-4"
          >
            <p className="text-sm paper-muted mb-1">{label}</p>
            <p className="text-2xl font-bold paper-title">{pct.toFixed(1)}%</p>
            <p className="text-xs paper-muted mt-1">
      ...
      <main className="container mx-auto px-4 py-10 sm:py-16">
        <div className="max-w-4xl mx-auto">
          ...
          <p className="text-text-light mb-10">
            How we ensure quality and reliability across the ESP32 & IoT storefront codebase.
          </p>
          ...
          <section className="mb-12">
            ...
            {coverageLoading ? (
              <div className="paper-surface rounded-xl p-6 paper-muted">
                Loading coverage…
              </div>
            ) : (
          ...
          <section>
            ...
            <div className="space-y-6">
              {TEST_FLOW_STEPS.map((step, i) => (
                <div
                  key={step.id}
                  className="paper-surface rounded-2xl p-6 transition-all duration-300 hover:-translate-y-1"
                >
                  <div className="flex items-start gap-4">
                    <span className="paper-icon-tile flex-shrink-0 w-10 h-10 rounded-full flex items-center justify-center font-bold">
                      {i + 1}
                    </span>
                    <div className="flex-1 min-w-0">
                      <h3 className="text-lg font-semibold paper-title mb-1">{step.title}</h3>
                      <p className="text-sm paper-muted mb-2">{step.tool}</p>
                      <p className="paper-title mb-4">{step.description}</p>
                      <ul className="space-y-1 text-sm paper-muted">
          ...
          <section className="paper-surface mt-12 rounded-2xl p-6">
            <h2 className="text-lg font-semibold paper-title mb-4">Commands</h2>
            <div className="space-y-2 font-mono text-sm">
              <p className="paper-title">
                <span className="paper-muted">$</span> npm run test
                <span className="paper-muted ml-2">— Single run</span>
              </p>
              <p className="paper-title">
                <span className="paper-muted">$</span> npm run test:watch
                <span className="paper-muted ml-2">— Watch mode</span>
              </p>
              <p className="paper-title">
                <span className="paper-muted">$</span> npm run test:coverage:public
                <span className="paper-muted ml-2">— Generate coverage and copy to Testing page</span>
              </p>
              <p className="paper-title">
                <span className="paper-muted">$</span> npm run test:e2e
                <span className="paper-muted ml-2">— Run Cypress E2E (headless)</span>
              </p>
              <p className="paper-title">
                <span className="paper-muted">$</span> npm run test:e2e:open
                <span className="paper-muted ml-2">— Open Cypress UI for interactive E2E testing</span>
              </p>
            </div>
          </section>
